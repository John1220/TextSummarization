...
class PacSumExtractor:
    ...
    def __init__(self, extract_num=1, beta=3, lambda1=-0.2, lambda2=-0.2):
    ...
    def tune_hparams(self, data_iterator, example_num=100):
...
class PacSumExtractorWithBert(PacSumExtractor):
    ...
    def __init__(self, bert_model_file, bert_config_file, extract_num=3, beta=3, lambda1=-0.2, lambda2=-0.2):
        ...
        self.model = self._load_edge_model_hugface(bert_model_file, bert_config_file)
    ...
    def _load_edge_model_hugface(self, bert_model_file, bert_config_file):
        from bert_model import BertEdgeScorer
        bert_config = BertConfig.from_json_file(bert_config_file)
        model = BertEdgeScorer(bert_config)
        # model_states = torch.load(bert_model_file)
        # print(model_states.keys())
